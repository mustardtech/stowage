...
**[stowage.dev](https://stowage.dev) · [Docs](https://stowage.dev/docs) · [Downloads](https://stowage.dev/download)**
...
**Dashboard.** OIDC or local accounts, an object browser with drag-and-drop multipart upload (pause/resume), previews, version history, tags, rename, cross-bucket move/copy, and download-as-zip.
...
One Go binary: an embedded SvelteKit frontend, and pure-Go SQLite for users, sessions, shares, audit events, and credentials. Secrets are sealed with AES-256-GCM under a master key. No Redis, no NATS, no external auth service — the tradeoffs (single-replica, SQLite-only) are deliberate; see [Why one binary](https://stowage.dev/docs/explanations/single-binary) and the [architecture overview](https://stowage.dev/docs/explanations/architecture).

Stowage does **not** store object bytes — data lives on the upstream; Stowage proxies access to it.

## Documentation

Canonical docs at [**stowage.dev/docs**](https://stowage.dev/docs), with sources in [`./docs/`](./docs/) for offline reading:

| Section | When to read |
|---|---|
| **[Getting started →](https://stowage.dev/docs/getting-started)** | "I'm new — teach me by doing." |
| **[Self-host →](https://stowage.dev/docs/self-host)** | Single-host recipes. |
| **[Run on Kubernetes →](https://stowage.dev/docs/kubernetes)** | Helm chart, operator, CRDs. |
| **[Use as an S3 endpoint →](https://stowage.dev/docs/s3-endpoint)** | Pointing AWS SDKs at the proxy. |
| **[Reference →](https://stowage.dev/docs/reference)** | Every flag, config key, endpoint, metric. |
| **[Explanations →](https://stowage.dev/docs/explanations)** | Architecture and design tradeoffs. |
| **[Security →](https://stowage.dev/docs/security)** | Threat model, hardening, reporting. |
| **[Comparisons →](https://stowage.dev/docs/comparisons)** | vs MinIO Console, Cyberduck, raw S3. |
...
[Website](https://stowage.dev) · [Blog](https://stowage.dev/blog) · [Source](https://github.com/stowage-dev/stowage) · [Discussions](https://github.com/stowage-dev/stowage/discussions) · [Issues](https://github.com/stowage-dev/stowage/issues)
